...
<template>
  <header>
    ...
    <div class="switchers">
      <NuxtLink :to="switchLocalePath(langNext[0])" class="toggle-lang" :aria-label="t('ariaLabel.lang-switch')" role="switch" aria-checked="true" @click="langTooltip">
        ...
        <span class="lang-tooltip" :class="{ active: isLangNext }">
          {{ langNext[1] }}
        </span>
      </NuxtLink>

      <button class="toggle-theme" type="button" role="switch" :aria-label="t(`ariaLabel.${theme}`)" aria-checked="true" @click="toggleTheme">
        <Transition name="toggle-theme" mode="out-in">
          <svg v-if="theme === 'theme-light'" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" role="img" aria-hidden="true">
            <circle cx="12" cy="12" r="5" />
            <path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42" />
          </svg>
          <svg v-else xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true">
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
          </svg>
        </Transition>
      </button>

      <button class="toggle-menu" type="button" role="switch" :aria-label="t(`ariaLabel.menu-${menuOpen ? 'close' : 'open'}`)" aria-checked="true" @click="toggleMenu">
        <span class="top" />
        <span class="middle" />
        <span class="bottom" />
      </button>
